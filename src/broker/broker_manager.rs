#[allow(dead_code)]
use crate::{
    connection::connection_manager::ConnectionManager, listener::listener::Listener,
    threaded::Threaded, threaded::ThreadedTask,
};
use std::{
    collections::HashMap,
    io::Result,
...
pub struct BrokerManager {
    ...
    connections: Arc<Mutex<HashMap<String, ConnectionManager>>>,
}
...
impl BrokerManager {
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        if let Some(threaded) = &mut self.threaded {
            ...
            Err(std::io::Error::new(
                ...
                "Threaded instance not initialized",
            ));
        }
        Ok(())
    }
    ...
    pub fn add_connection(&mut self, address: &String) -> Result<()> {
        ...
        connections_guard.insert(
            address.to_string(),
            ConnectionManager::new(&address.to_string())
                .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, format!("{:?}", e)))?,
        );
...
impl ThreadedTask for BrokerManager {
    fn initialize(&mut self) -> Result<()> {
        // Start the listener
        self.listener
            .lock()
